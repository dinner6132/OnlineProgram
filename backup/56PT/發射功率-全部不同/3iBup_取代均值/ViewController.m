...
- (void)viewDidLoad {
    ...
    self.AppVersionLabel.text = @"均值,取代-90,3upiB";
...
//        });
//    }
    if (signalThreshold >= 3) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.pointLabel.text = @"計算中...";
            //NSLog(@"sth=%d",signalThreshold);
...
//        for (int j=0; j<7; j++) {
//            NSLog(@"1 s_g[1][%d]=%f",j,signal_filter[1][j]);
//        }
//        
//        for (int j=0; j<7; j++) {
//            NSLog(@"1 s_g[3][%d]=%f",j,signal_filter[3][j]);
//        }
//        
//        NSLog(@"\n");
//        
//        for (int j=0; j<4; j++) {
//            NSLog(@"1 avg[%d]=%f",j,sg_temp[j]);
//        }
//        
//        NSLog(@"\n");
        ...
        for (int j=0; j<7; j++) {
            NSLog(@"2 s_g[1][%d]=%f",j,signal_filter[1][j]);
        }
//        for (int j=0; j<7; j++) {
//            NSLog(@"2 s_g[3][%d]=%f",j,signal_filter[3][j]);
//        }
...
//    NSLog(@"\n");
    ...
    for (int i=0; i<4; i++) {
        for (j=0; j<56; j++) {
            if (iB_on[i] >= -90) {
                pow_temp[i][j] = pow((iB_on[i]-iB_of_AVG[i][j]),2);
            }
        }
    }
    for (int i=0; i<4; i++) {
        for (j=0; j<56; j++) {
            temp = sqrt(pow_temp[0][j] + pow_temp[1][j] + pow_temp[2][j] + pow_temp[3][j]);
            dis[j] = temp;
        }
    }
